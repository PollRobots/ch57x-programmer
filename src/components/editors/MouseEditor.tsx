  ...
  return (
    <div className="flex flex-row gap-2">
        ...
        <SimpleRadio
          ...
          onChange={({ value }) => {
            switch (value) {
              case "Move":
              case "Drag":
                onClick({ type: value, x: 0, y: 0, buttons: [] });
                break;
              case "Click":
                onClick({ type: value, buttons: [] });
                break;
              case "Wheel":
                onClick({ type: value, delta: 0 });
                break;
            }
          }}
      ...
      {(action?.type === "Move" || action?.type === "Drag") && (
        <div className="flex flex-col gap-2">
          <Text size="sm">Move vector</Text>
          <div className="grid size-36 bg-white dark:bg-neutral-900">
            <Text size="xs" className="col-start-1 row-start-1">
              ({action.x}, {action.y})
            </Text>
      ...
      {action?.type === "Wheel" && (
        <div className="flex flex-col gap-2">
          <Text size="sm">{action.type} vector</Text>
          <div className="grid size-36 bg-white dark:bg-neutral-900">
            <Text size="xs" className="col-start-1 row-start-1">
              ({action.delta})
            </Text>
      ...
      {(action?.type === "Click" ||
        ...
        <>
          <div className="flex flex-col gap-2">
            ...
            <MouseButtons
              buttons={"buttons" in action ? action.buttons : []}
              onChange={update => {
                if (action.type === "Click") {
                  onClick({ ...action, buttons: update });
                } else {
...
function MouseButtons({ buttons, onChange }: MouseButtonsProps) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <Button
        variant="invisible"
        className={mousebuttons({ selected: buttons.includes("Left") })}
        onClick={() => toggle("Left")}
      >
      ...
      <Button
        variant="invisible"
        className={mousebuttons({ selected: buttons.includes("Middle") })}
      ...
      <Button
        variant="invisible"
        className={mousebuttons({ selected: buttons.includes("Right") })}
